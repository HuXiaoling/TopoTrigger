...
def extract_fv(model_filepath,examples_dirpath,example_img_format='png',model_id=0):
    ...
    cc = torch.zeros(nclasses,rounds,nchkpt);
    cc_size = torch.zeros(nclasses,rounds,nchkpt);
        ...
        if not (ims[c] is None):
            ...
            for round in range(rounds):
                ...
                for k in range(nsteps+delta):
                    ...
                    mask = torch.sigmoid(a * multi)[0][0]

                    if k-delta>50:
                        loss_topo = 10 * getTopoLoss(-mask)
                        loss = loss_topo
                        loss_values.append(loss)
                        loss = loss_target + loss_a + loss_div + 10 * loss_topo;
                    else:
                        loss = loss_target + loss_a + loss_div;
...
if __name__ == "__main__":
    ...
    id_list=torch.randperm(1).tolist();
    if params.start>=0:
        if params.end>=0:
